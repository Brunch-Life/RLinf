...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_realagainst_peg_slot",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=True,
        ...
        data=LeRobotRLTManiSkillJointDataConfig(
            repo_id="local/realagainst_peg_slot",
            base_config=DataConfig(prompt_from_task=False),
            assets=AssetsConfig(
                assets_dir="checkpoints/torch/pi05_realagainst_peg_slot/assets"
            ),
            extra_delta_transform=False,
            default_prompt="insert the grasped peg downward into the movable slot",
            output_action_dim=9,
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "checkpoints/jax/pi05_base"
