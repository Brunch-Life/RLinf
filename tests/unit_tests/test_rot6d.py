...
def test_matrix_rot6d_round_trip_single():
    for R_mat in _random_rotations(N).as_matrix():
        r6 = matrix_to_rot6d(R_mat)
        assert r6.shape == (6,)
        R_rec = rot6d_to_matrix(r6)
        assert np.allclose(R_rec, R_mat, atol=TOL)
...
def test_gram_schmidt_robust_to_noise():
    """Adding small noise to rot6d still yields a valid SO(3)."""
    rng = np.random.default_rng(42)
    r6 = matrix_to_rot6d(_random_rotations(N, rng).as_matrix())
    r6_noisy = r6 + rng.normal(scale=1e-3, size=r6.shape).astype(np.float32)
    R_rec = rot6d_to_matrix(r6_noisy).astype(np.float64)
    dets = np.linalg.det(R_rec)
    assert np.all(np.abs(dets - 1.0) < 1e-5), (
        f"dets drift under noise: range [{dets.min()}, {dets.max()}]"
    )
    eye = np.einsum("...ji,...jk->...ik", R_rec, R_rec)
...
def test_SE3_delta_is_identity_when_state_equals_abs():
    xyz = np.array([0.5, -0.2, 0.3])
    r6 = matrix_to_rot6d(
        R.from_euler("xyz", [0.1, -0.3, 0.7]).as_matrix()
    )  # scipy.from_euler is OK INSIDE tests only; rot6d module itself never uses euler
    T = pose_to_SE3(xyz, r6)
    delta = se3_body_delta(T, T)
    assert np.allclose(delta, np.eye(4), atol=1e-8)
