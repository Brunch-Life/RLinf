...
def test_expand_multi_view_images_none():
    ...
    assert CollectEpisode._expand_multi_view_images("wrist_image", None) == {}


def test_expand_multi_view_images_dual_view():
    """[N, H, W, C] (N>1) fans out to indexed keys with content preserved."""
    ...
    assert set(result) == {"wrist_image-0", "wrist_image-1"}
...
def test_collect_image_keys_dual_wrist():
    """Indexed wrist keys are collected with their (H, W, C) shapes."""
    frame = {
    ...
    assert result == {"wrist_image-0": (64, 64, 3), "wrist_image-1": (64, 64, 3)}
...
def test_realworld_env_dual_branch_emits_semantic_keys(dual_realworld_env):
    """Dual-arm shares the single-arm path: main_image_key picks the primary frame,
    the rest stacks into extra_view_images. State is gripper(2)+force(6)+pose(12 quat)
    +torque(6)+vel(12) = 38 (DualQuat2EulerWrapper still wraps even with use_relative_frame=False)."""
    obs, _ = dual_realworld_env.reset()
    assert "main_images" in obs and "extra_view_images" in obs
    assert "wrist_images" not in obs
    assert "left_wrist_images" not in obs and "right_wrist_images" not in obs
    assert obs["states"].shape[-1] == 38

    next_obs, *_ = dual_realworld_env.step(np.zeros((1, 14), dtype=np.float32))
    assert "main_images" in next_obs and "extra_view_images" in next_obs
