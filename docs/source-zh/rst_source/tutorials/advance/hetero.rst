...
默认情况下，Franka 硬件节点使用 Intel RealSense 相机和 Franka 内置平行指夹爪。
你可以通过在 ``configs`` 中添加以下字段，切换为 **ZED 相机** 和/或 **Robotiq 2F 夹爪**：
	    ...
	    - label: franka
	      ...
	      hardware:
	        ...
	        configs:
	          - robot_ip: "172.16.0.2"
	            node_rank: 0
	            camera_type: "zed"               # "realsense"（默认）或 "zed"
	            camera_serials:
	              - "12345678"
	            gripper_type: "robotiq"           # "franka"（默认）或 "robotiq"
	            gripper_connection: "/dev/ttyUSB0" # Robotiq 的 USB-RS485 串口

**ZED 相机**

* 从 `Stereolabs 官网 <https://www.stereolabs.com/developers/release>`_
  下载并安装 ZED SDK。SDK 安装后会自带 ``pyzed`` Python 包（绑定到系统
  Python），不能通过 pip 安装。
* 在硬件配置中设置 ``camera_type: "zed"``。
* ZED 序列号是较短的纯数字字符串，可通过
  ``ZEDCamera.get_device_serial_numbers()`` 或 ``ZED Explorer`` 工具查看。

**Robotiq 2F 夹爪**

* 通过 **USB-RS485 转接器** 将夹爪连接到控制 PC（例如 ``/dev/ttyUSB0``）。
* ``pymodbus`` 包（随 ``franka`` 依赖组自动安装）通过 Modbus RTU 协议直接
  控制夹爪——\ **不需要额外安装 ROS 包**\ 。
* 设置 ``gripper_type: "robotiq"`` 并将 ``gripper_connection`` 指向串口路径。
* 使用 Robotiq 时，阻抗控制器会自动以 ``load_gripper:=false`` 启动，
  避免 Franka 尝试管理内置夹爪。
* 请记得在 `Franka Desk <https://franka.de>`_ 中配置末端执行器负载参数，
  以匹配 Robotiq 的质量（2F-85 为 0.925 kg）和质心位置。