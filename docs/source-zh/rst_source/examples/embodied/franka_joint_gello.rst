...
- ``franky`` 将 1 kHz ``robot.control()`` 循环和阻抗力矩计算完全运行在
  C++ ``std::thread`` 中。
...
详见 ``requirements/embodied/franky_install.md``。
...
- 未应用开机 RT 调优，或 ``/etc/security/limits.d/`` 未授予用户
  ``rtprio >= 80``。请参照 ``requirements/embodied/franky_install.md``。
...
``FrankyController`` 使用 ``JointImpedanceTrackingMotion`` 进行流式发送，
通过阻抗力矩平滑跟踪最新参考位置，无需每次调用都重规划轨迹。如果仍有嗡嗡声：
