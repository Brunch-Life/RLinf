...
2. **对齐数据处理流程：** OpenPI 侧的 ``pi0_custom`` 配置需要与 RLinf 的数据处理保持一致。主要涉及以下修改（请确保你的 OpenPI 版本已包含这些改动）：
...
.. list-table::
   ...
   * - **数据采集** (``collect_data_with_wrapper.sh``)
     ...
     - 失败并结束 episode
     - 成功并结束 episode
   * - **评估** (``eval_realworld.sh``, single_stage)
     - 失败 (-1, 结束)
     - 中性 (0, 不结束)
     - 成功 (+1, 结束)
...
.. list-table::
   ...
   * - ``rlinf/models/embodiment/openpi/dataconfig/__init__.py``
     - RLinf 侧 ``pi0_custom`` 配置定义
   * - ``openpi/src/openpi/policies/franka_policy.py``
     - OpenPI 侧 FrankaEEInputs（需与 RLinf 对齐）
   * - ``openpi/src/openpi/training/config.py``
     - OpenPI 侧 ``pi0_custom`` 训练配置（需与 RLinf 对齐）
