...
# <your gello_software clone>/gello/agents/gello_agent.py
# (replacing the existing placeholder GELLO unit entry), then re-run
...
SRC_FILE="${REPO_PATH}/toolkits/realworld_check/test_gello.py"

export PYTHONPATH="${REPO_PATH}:${PYTHONPATH:-}"
export FRANKA_ROBOT_IP="${FRANKA_ROBOT_IP:-172.16.0.2}"
export GELLO_PORT="${GELLO_PORT:-/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_YOURGELLO-if00-port0}"
...
exec python "${SRC_FILE}" calibrate
